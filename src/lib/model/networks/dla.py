...
class BasicBlock(nn.Module):  # BasicBlock和Bottleneck和ResNet中的一致
...
# BottleneckX实际上是ResNeXt中的基础模块，也可以作为DLA中的基础模块。
# DLA34中调用的依然是BasicBlock
...
class DLA(nn.Module):
    ...
    def forward(self, x, pre_img=None, pre_hm=None):   # 这里定义三个输入怎么操作
        y = []
        x = self.base_layer(x)
